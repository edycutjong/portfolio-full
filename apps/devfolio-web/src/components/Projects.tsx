        ...
        <section id="projects" className="section-padding px-6">
            <div className="container-wide">
                ...
                <div className="text-center mb-8 animate-fade-in">
                    <h2 className="text-3xl md:text-4xl font-bold mb-4">Production Projects</h2>
                ...
                <div className="flex flex-wrap justify-center gap-3 mb-14">
                    {categories.map((cat) => (
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${activeCategory === cat.id
                ...
                {featuredProjects.length > 0 && (
                    <div className="grid md:grid-cols-3 gap-8 mb-10 stagger">
                        {featuredProjects.map((project) => (
                            <article
                                key={project.id}
                                className="card card-featured p-7 flex flex-col"
                            >
                                {/* Header */}
                                ...
                                {/* Content */}
                                <h3 className="text-xl font-bold mb-1.5">{project.name}</h3>
                                <p className="text-cyan-400 text-sm mb-4">{project.tagline}</p>
                                <p className="text-muted text-sm mb-5 grow leading-relaxed">{project.description}</p>

                                {/* Impact */}
                                <div className="text-xs text-violet-400 mb-5 font-mono">
                                    → {project.impact}
                                </div>
                                ...
                                {/* Links */}
                                <div className="flex gap-5 mt-auto pt-5 border-t border-zinc-800">
                                    {project.github && (
                                        <a
                                            href={project.github}
                                            target="_blank"
                                    ...
                                    {project.demo && (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="flex items-center gap-2 text-sm text-muted hover:text-cyan-400 transition-colors"
                                        >
                                    ...
                                    {project.docs && (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="flex items-center gap-2 text-sm text-muted hover:text-violet-400 transition-colors"
                                        >
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 stagger">
                        {otherProjects.map((project) => (
                            ...
                            >
                                ...
                                <p className="text-sm text-muted mb-5 grow leading-relaxed">{project.description}</p>

                                <div className="flex flex-wrap gap-2 mb-5">
                                    {project.techStack.slice(0, 4).map((tech) => (
                                        <span key={tech} className="text-xs px-2.5 py-1 bg-zinc-800 rounded text-zinc-400">
                                            {tech}
                                        </span>
                                    ))}
                                </div>

                                <div className="flex gap-5 mt-auto pt-4 border-t border-zinc-800">
                                    {project.github && (
                                        <a
                                            href={project.github}
                                            target="_blank"
